...
async function main() {
  ...
  if (!CRON_SECRET) {
    ...
    // Your campaigns route likely expects secret via header or query;
    // if your implementation uses header 'x-cron-secret', keep this.
    // If your route uses Authorization Bearer, tell me and I'll adjust.
    const r4 = await hit("api/campaigns/run (GET)", "/api/campaigns/run", {
      method: "GET",
      headers: hdrs({ "x-cron-secret": CRON_SECRET }),
    });
    logResult(r4);
    // don't fail the whole run if this route expects a different auth style
  }
